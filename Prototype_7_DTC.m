...
simulationTime = inf; % Variable to store simulation time
    ...
    function drawConclusions(heartRateData, walkingRateData)
        % Classify activity using the trained Decision Tree model
        ...
        model = py.joblib.load('./Models/decision_tree_model.pkl');
        ...
        % Print the current data and predicted activity
        fprintf('Current Heart Rate: %.2f bpm\n', currentHR);
        fprintf('Current Walking Rate: %.2f steps/min\n', currentWR);
